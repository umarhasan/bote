...
<div class="page-wrapper">
    ...
    <div class="row page-titles">
        <div class="col-md-5 align-self-center">
            <h3 class="text-themecolor">{{trans('lang.vehicle_brand_table')}}</h3>
        </div>
        <div class="col-md-7 align-self-center">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{url('/dashboard')}}">{{trans('lang.dashboard')}}</a></li>
                <li class="breadcrumb-item active">{{trans('lang.vehicle_brand_table')}}</li>
            </ol>
        </div>
    </div>
                        ...
                        <ul class="nav nav-tabs align-items-end card-header-tabs w-100">
                            <li class="nav-item">
                                <a class="nav-link active" href="{!! url()->current() !!}"><i
                                        class="fa fa-list mr-2"></i>{{trans('lang.vehicle_brand_table')}}</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" href="{{route('vehicle-brand.create')}}"><i
                                        class="fa fa-plus mr-2"></i>{{trans('lang.brand_add')}}</a>
                            </li>
                        </ul>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="table-responsive m-t-10">
                            <table id="taxTable"
                                ...
                                <thead>
                                    <tr>
                                        <?php if (in_array('vehicle.brand.delete', json_decode(@session('user_permissions')))) { ?>
...
<script type="text/javascript">
    ...
    if ($.inArray('vehicle.brand.delete', user_permissions) >= 0) {
    ...
    async function getListData(val) {
        ...
        newdate = '';
        var id = val.id;
        var route1 = '{{route("vehicle-brand.edit", ":id")}}';
    ...
    $(document).on("click", "input[name='isEnabled']", function (e) {
        ...
        var id = this.id;
        if (ischeck) {
            database.collection('vehicle_brand').doc(id).update({ 'enable': true }).then(function (result) {

            });
        } else {
            database.collection('vehicle_brand').doc(id).update({ 'enable': false }).then(function (result) {
